...
def register():
    if request.method == 'POST':
        ...
        if app.config['MAIL_USERNAME'] and app.config['MAIL_PASSWORD']:
            try:
                msg = Message('Подтверждение регистрации на Comilank', recipients=[email])
            ...
            print(f"\n Код для {email}: {code}\n")
            flash(f'Код (в консоли): {code}', 'success')
...
def ban_user(user_id):
    ...
    if not current_user.can_ban(target_user):
        ...
        return redirect(url_for('admin_users'))
    forever      = request.form.get('forever') == 'on'
...
def delete_user(user_id):
    ...
    if target_user.id == current_user.id:
        ...
        return redirect(url_for('admin_users'))
    db.session.delete(target_user); db.session.commit()
