...
# ========== YOUTUBE API ==========
# Ключ API – обязательно из переменных окружения (на Render)
YOUTUBE_API_KEY = os.environ.get('YOUTUBE_API_KEY')
if not YOUTUBE_API_KEY:
    # ⚠️ Для локальной разработки можно указать ключ здесь, но перед деплоем удали!
    YOUTUBE_API_KEY = 'AIzaSyCbJF2Jl2AMdDXemcir4KVnTBJdx3rFUTA'  # замени на свой для локального теста

# ID стримов – теперь берутся из переменных окружения (задаются на Render)
# Если переменная не задана, используется пустая строка (вернёт 0 зрителей)
VIDEO_ID_SHORTS = os.environ.get('VIDEO_ID_SHORTS', '')      # YouTube Shorts
VIDEO_ID_HORIZONTAL = os.environ.get('VIDEO_ID_HORIZONTAL', '')  # Горизонтальный стрим
...
# ========== ФУНКЦИЯ ДЛЯ ПОЛУЧЕНИЯ ДАННЫХ О СТРИМЕ ПО ID ==========
