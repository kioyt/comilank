...
def api_check_vpn_debug():
    """Отладка VPN - только для администраторов."""
    from datetime import datetime as _dt
...
def _init_weather_cities():
    """Создаёт стандартные города при первом запуске если их нет."""
    try:
...
def api_weather():
    """Возвращает реальную погоду для всех активных городов через Open-Meteo (бесплатно, без ключа)."""
...
# ── IP-бан через профиль пользователя ──
...
# ── Сброс пароля через email (код) ──
def _send_reset_code(user):
    """Генерирует 6-значный код, сохраняет в БД, отправляет на почту. Возвращает (ok, code)."""
...
def forgot_password():
    """Шаг 1: пользователь вводит логин - получает код на почту."""
    error = None
    if request.method == 'POST':
        username = request.form.get('username', '').strip()
        user = User.query.filter(func.lower(User.username) == func.lower(username)).first()
...
def reset_password_code():
    """Шаг 2: пользователь вводит код + новый пароль."""
...
def admin_home_moment_bulk():
    """Массовая загрузка моментов - несколько файлов сразу."""
...
def admin_home_moment_edit(mid):
    """Редактирование существующего момента стрима."""
...
# ========== АВТОМИГРАЦИЯ (запускается при старте) ==========
def _run_migration():
    """Добавляет недостающие колонки и таблицы. Безопасно вызывать многократно."""
