...
def create_tables():
    ...
    with db.engine.connect() as conn:
        for sql in [
            "ALTER TABLE users ADD COLUMN recovery_key VARCHAR(256)",
            "ALTER TABLE users ADD COLUMN avatar VARCHAR(300)",
            "ALTER TABLE articles ADD COLUMN video_file VARCHAR(300)",
        ]:
            try:
        ...
        try:
            ...
            conn.execute(db.text(
                """CREATE TABLE IF NOT EXISTS extra_page_views (
...
@app.route('/api/vpn-report', methods=['POST'])
def api_vpn_report():
    """Принимает отчёт о VPN от клиента и сохраняет в БД."""
    try:
        ...
        # Не дублируем — один IP за последние 24 часа
        from datetime import timedelta
        since = datetime.utcnow() - timedelta(hours=24)
        exists = db.session.execute(
            db.text("SELECT id FROM vpn_logs WHERE ip_address=:ip AND detected_at > :since LIMIT 1"),
            {'ip': ip, 'since': since}
        ).fetchone()
        if not exists:
            db.session.execute(
                db.text("INSERT INTO vpn_logs (user_id, username, ip_address, vpn_type, country, detected_at) "
                        "VALUES (:uid, :uname, :ip, :vtype, :country, :now)"),
                {'uid': user_id, 'uname': username, 'ip': ip,
                 'vtype': vpn_type, 'country': country, 'now': datetime.utcnow()}
            )
            db.session.commit()
...
def admin_vpn_logs():
    ...
    rows = db.session.execute(
        db.text("SELECT username, ip_address, vpn_type, country, detected_at "
                "FROM vpn_logs ORDER BY detected_at DESC LIMIT 100")
    ).fetchall()
    result = [{
        'username': r[0] or 'Гость',
        'ip': r[1], 'vpn_type': r[2],
        'country': r[3], 'detected_at': str(r[4])
    } for r in rows]
