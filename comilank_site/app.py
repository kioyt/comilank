...
from models import (db, User, Game, Article, Comment, Vote, DropdownItem,
                    ...
                    ArticleView, CommentReaction, SiteSettings)
...
def create_tables():
    db.create_all()
    # Автомиграция: добавляем recovery_key если нет
    try:
        with db.engine.connect() as conn:
            conn.execute(db.text(
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS recovery_key VARCHAR(256)"
            ))
            conn.execute(db.text(
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar VARCHAR(300)"
            ))
            # Автомиграция: таблица настроек сайта
            conn.execute(db.text(
...
def record_article_view(article):
    """Засчитывает просмотр один раз на пользователя (по user_id) или на IP (если не авторизован).
    Если пользователь уже смотрел статью как гость (по IP), и теперь вошёл — новый просмотр НЕ засчитывается."""
    ip = get_client_ip()
    if current_user.is_authenticated:
        # Проверяем по user_id
        existing_user = ArticleView.query.filter_by(article_id=article.id, user_id=current_user.id).first()
        if existing_user:
            return  # уже смотрел как залогиненный
        # Проверяем — может смотрел как гость с этого IP
        existing_ip = ArticleView.query.filter_by(article_id=article.id, ip_address=ip, user_id=None).first()
        if existing_ip:
            # Привязываем тот просмотр к пользователю, не добавляем новый
