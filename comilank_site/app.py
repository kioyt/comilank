...
def api_check_vpn():
    ...
    try:
        ...
        # Попытка 1: ip-api.com (proxy поле только в платной, но hosting хорошо работает)
        try:
            r1 = requests.get(
            ...
            d1 = r1.json()
            if d1.get('status') == 'success' and (d1.get('proxy') or d1.get('hosting')):
                is_vpn = True
        except Exception:
            pass
        ...
        # Попытка 3: proxycheck.io (без ключа — 100/день, но как fallback)
        if not is_vpn:
            try:
                r3 = requests.get(
