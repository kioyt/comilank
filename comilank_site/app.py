...
def create_tables():
    ...
    with db.engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(db.text(
                f"""CREATE TABLE IF NOT EXISTS vpn_logs (
                    id          {_id_col},
                    user_id     INTEGER,
                    ip_address  VARCHAR(45)  DEFAULT '',
                    vpn_type    VARCHAR(50)  DEFAULT 'VPN',
...
def api_check_vpn():
    ...
    try:
        ...
        try:
            r1 = requests.get(
                f'http://ip-api.com/json/{ip}?fields=status,proxy,hosting,query,country',
                timeout=3
            )
            d1 = r1.json()
            if d1.get('status') == 'success':
                country = d1.get('country', '')
                if d1.get('proxy') or d1.get('hosting'):
                    is_vpn = True
                    vpn_type = 'Proxy' if d1.get('proxy') else 'Hosting/VPN'
        except Exception:
            pass
        ...
        # Логируем в БД если VPN обнаружен
        if is_vpn:
            try:
