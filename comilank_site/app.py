    ...
    if lat is not None and lon is not None:
        try:
            url = (f'https://api.open-meteo.com/v1/forecast'
                   ...
                   f'&current=temperature_2m,weathercode,windspeed_10m,is_day'
                   f'&wind_speed_unit=ms&timezone={tz}&forecast_days=1')
            ...
            return jsonify(cities=[{
                'name': name, 'temp': round(cur.get('temperature_2m', 0), 1),
                'wind': round(cur.get('windspeed_10m', 0), 1),
                'code': code, 'desc': desc, 'css': css,
                'is_day': cur.get('is_day', 1),
            }])
        except Exception:
            return jsonify(cities=[{'name': name, 'temp': None, 'wind': 0, 'code': 0, 'desc': '-', 'css': 'clear', 'is_day': 1}])

    # Multi-city mode (legacy): all cities from DB
    ...
    for city in cities:
        try:
            url = (f'https://api.open-meteo.com/v1/forecast'
                   ...
                   f'&current=temperature_2m,weathercode,windspeed_10m,is_day'
                   f'&wind_speed_unit=ms&timezone={city.tz}&forecast_days=1')
            resp = requests.get(url, timeout=5)
            data = resp.json()
            cur  = data.get('current', {})
            code = cur.get('weathercode', 0)
            ...
            result.append({'id': city.id, 'name': city.name,
                'temp': None, 'wind': None, 'desc': '-', 'css': 'clear', 'is_day': 1})
...
def _run_migration():
    ...
    _new_tables = [
        ...
        f"""CREATE TABLE IF NOT EXISTS battle_invites (id {_idc}, from_id INTEGER NOT NULL, to_id INTEGER NOT NULL, room_id VARCHAR(32) NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, accepted BOOLEAN DEFAULT FALSE)""",
    ]
    try:
...
def api_game_battle_invite():
    ...
    try:
        db.session.execute(db.text(
            "INSERT INTO battle_invites (from_id, to_id, room_id, created_at, accepted) "
            "VALUES (:f, :t, :r, :dt, FALSE)"
...
def api_game_battle_check():
    ...
    try:
        ...
        row = db.session.execute(db.text(
            "SELECT bi.id, bi.room_id, u.username "
...
def api_game_battle_respond():
    ...
    try:
        if accept:
            db.session.execute(db.text(
                "UPDATE battle_invites SET accepted=TRUE WHERE id=:id AND to_id=:uid"
...
def api_game_battle_status(room_id):
    ...
    try:
        row = db.session.execute(db.text(
            "SELECT accepted, to_id FROM battle_invites WHERE room_id=:r AND from_id=:uid"
