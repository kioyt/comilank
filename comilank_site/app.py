...
def create_tables():
    ...
    with db.engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(db.text(
                """CREATE TABLE IF NOT EXISTS extra_page_comments (
                    id SERIAL PRIMARY KEY,
            ...
            conn.execute(db.text(
                """CREATE TABLE IF NOT EXISTS extra_comment_reactions (
                    id SERIAL PRIMARY KEY,
            ...
            conn.execute(db.text(
                """CREATE TABLE IF NOT EXISTS extra_page_votes (
                    id SERIAL PRIMARY KEY,
                    page_id INTEGER NOT NULL,
                    user_id INTEGER NOT NULL,
                    value INTEGER NOT NULL,
...
def add_extra_comment(page_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO extra_page_comments (content, page_id, author_id, parent_id) VALUES (?, ?, ?, ?)",
            (content, page_id, current_user.id, parent_id)
        )
        ...
        try:
            db.session.execute(
                db.text("INSERT INTO extra_page_comments (content,page_id,author_id,parent_id) "
                        "VALUES (:c,:pid,:aid,:par)"),
                {'c': content, 'pid': page_id, 'aid': current_user.id, 'par': parent_id}
            )
            db.session.commit()
            row = db.session.execute(
                db.text("SELECT id FROM extra_page_comments WHERE page_id=:pid AND author_id=:aid "
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify(ok=True, id=comment_id,
                       ...
                       content=content,
                       parent_id=parent_id)
    anchor = f'#ec-replies-{parent_id}' if parent_id else f'#ec-{comment_id}'
