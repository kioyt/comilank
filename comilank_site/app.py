...
def api_weather():
    ...
    # Маппинг wttr.in weatherCode (WW-коды) -> наш тип + описание
    def _parse_wttr(data):
        """Парсим ответ wttr.in и возвращаем dict с temp/wind/code/desc/is_day."""
        try:
            cur = data['current_condition'][0]
            temp = float(cur.get('temp_C', 0))
            wind = round(float(cur.get('windspeedKmph', 0)) / 3.6, 1)  # km/h -> m/s
            ...
            # WW code -> WMO-like code для совместимости с getWeatherType на фронте
            def ww_to_wmo(ww):
                if ww in (113,):            return 0   # Clear
                if ww in (116,):            return 1   # Partly cloudy
                if ww in (119, 122):        return 3   # Cloudy/overcast
                if ww in (143, 248, 260):   return 45  # Fog
                if ww in (176, 263, 266, 293, 296):  return 61  # Light rain
                if ww in (299, 302, 305, 308):        return 63  # Mod/heavy rain
                if ww in (281, 284, 185, 182):        return 51  # Drizzle
                if ww in (179, 323, 326, 329, 332, 335, 338, 368, 371): return 73  # Snow
                if ww in (311, 314, 317, 320):  return 71  # Light snow
                if ww in (227, 230):            return 86  # Blizzard
                if ww in (200, 386, 389, 392, 395): return 95  # Thunderstorm
                if ww in (362, 365, 374, 377):  return 80  # Showers
                return 0
            code = ww_to_wmo(ww)
            # Human-readable desc in Russian
            desc_map = {
                0:'Ясно', 1:'Переменная облачность', 3:'Облачно',
                45:'Туман', 51:'Морось', 61:'Дождь', 63:'Дождь',
                65:'Ливень', 71:'Снег', 73:'Снег', 80:'Ливень',
                86:'Метель', 95:'Гроза'
            }
            desc_ru = desc_map.get(code, desc_en or 'Ясно')
            return {'temp': round(temp, 1), 'wind': wind, 'code': code,
                    'desc': desc_ru, 'is_day': is_day}
        except Exception:
    ...
    def _fetch_city(lat, lon, name):
        try:
            ...
            r = requests.get(url, timeout=10, headers={'User-Agent': 'comilank-weather/1.0'})
            data = r.json()
            d = _parse_wttr(data)
            d['name'] = name
            return d
    ...
    # Single-city mode: ?lat=&lon=&tz=&name=
    ...
    # Multi-city mode: all cities from DB, parallel requests via wttr.in
...
def admin_home_next_stream():
    ...
    try:
        ns.preview_url = request.form.get('preview_url', '').strip()
    except Exception:
